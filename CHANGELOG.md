...
## v0.9.3 — 2026-03-19 · 邻接加成体验升级 — 六维度成就系统 & 全局统计

> **Adjacency Experience Upgrade**
> 为邻接加成系统引入完整的六维度体验升级，包括成就殿堂、分数计算、统计面板、实时反馈和排行系统，将邻接从隐性机制提升为显性战略核心。
> 代码变更：game.js +156/-0 行，index.html +0/-0 行

### ✨ 新功能 — 邻接成就系统（6个新成就 + 新类别）
- [office] 新增 **邻接** 成就分类（🔗 图标），包含 6 个阶梯式成就：
  - 🥉 **初次共振** — 首个建筑获得邻接加成（奖励：30⚡20🟢）
  - 🥈 **供给链达人** — 5 栋建筑同时有邻接加成（奖励：60⚡15🧬）
  - 🥈 **空间规划师** — 总邻接加成达 50%（奖励：80⚡20🧬10🍖）
  - 🥇 **完美邻接** — 单栋建筑邻接加成 ≥30%（奖励：120⚡30🧬15🍖）
  - 🥇 **协同大师** — 总邻接加成达 100%（奖励：150⚡40🧬20🍖）
  - 💎 **共生网络** — 所有已建建筑都有邻接加成（奖励：200⚡60🧬30🍖）

### ✨ 新功能 — 分数计算纳入邻接加成
- [office] calcScore() 新增第 11 计分维度：**每 10% 总邻接加成 = 25 分**，封顶 200 分
- [office] 排行榜、评级说明、分享卡片均已同步更新邻接维度

### ✨ 新功能 — 全局邻接统计面板
- [office] **统计面板**：新增 `🔗 邻接加成 ×N栋 · +XX%` 一行，有加成时亮绿色显示
- [office] **供应链面板**：顶部新增邻接统计行 + 最强建筑信息

### ✨ 新功能 — 邻接加成变化飘字
- [office] 放置建筑后如果提升全局邻接加成，从放置格子位置飘出绿色提示：`🔗 总邻接 +12% → +18%`
- [office] 新增 `_showAdjFloat()` 函数，基于 `_showScoreFloat()` 复用代码

### ✨ 新功能 — 建筑邻接 Top 5 排行
- [office] 供应链面板显示邻接加成 Top 5 建筑排行：🥇🥈🥉④⑤ + 名称 + 加成百分比
- [office] **点击可高亮跳转**到对应格子位置，新增 `_focusCell(idx)` 辅助函数
- [office] 加成百分比按阈值着色：30%+金色、15~30%亮绿、<15%蓝色

### 🎯 技术实现亮点
- [office] `_getAdjStats()` 带 5 秒缓存，避免每帧重复遍历全部格子
- [office] 放置/回收建筑时 `_invalidateAdjStats()` 强制刷新缓存
- [office] 供应链 hash 计算纳入邻接数据，确保面板实时更新
- [office] 邻接统计与现有成就检测、分数计算、UI 面板无缝集成
