...
// ===== AUDIO ENGINE — 已拆分至 sfx.js =====
// const SFX 定义在 sfx.js 中，通过 <script src="sfx.js"> 加载
...
// P3: manual（旧管线在升级时已全转手动，不再退化）
function getBeltMode() {
  if (G.phase <= 1) return 'auto';
  if (G.phase >= 3) return 'manual';
  // P2 渐退三拍
...
const GUIDE = {
  ...
  3: [
    ...
    { check: s => s._p3LogisticsTutorialPending, text:'🔧 物流时代｜菌主大人，旧管线已保留为手动连接！从现在起，新建筑需要你手动连线。端口是你的核心约束——合理分配每条管线！选中建筑→点击目标→连接', icon:'🔗', once:'p3LogisticsTutorial' },
...
const G = {
  ...
  _renderBeltTypeList(groups) {
    ...
    groups.forEach((grp, grpIdx) => {
      ...
      // ★ P3旧管线标记——检查该组是否包含从自动转换来的旧管线
      let legacyTag = '';
      if (this._p3ConvertedBelts && this._p3ConvertedBelts.size > 0) {
        const legacyCount = grp.keys.filter(k => this._p3ConvertedBelts.has(k)).length;
        if (legacyCount > 0) {
          legacyTag = ` <span style="font-size:0.75em;padding:1px 4px;border-radius:3px;background:rgba(251,146,60,0.25);color:#fb923c">旧</span>`;
        }
      }
      ...
      item.innerHTML = `
        <div style="flex:1;min-width:0">
          <div class="belt-type-route">${resIcons} ${grp.fromName} → ${grp.toName}${countTag}${legacyTag}</div>
  ...
  confirmRecycle() {
    ...
    for (const key of Object.keys(this.manualBelts)) {
      ...
      if (a === idx || b === idx) {
        ...
        if (this._p3ConvertedBelts) this._p3ConvertedBelts.delete(key);
      }
    }
  ...
  _tickEvoTask() {
    ...
    if (task.id === 'et1_econ30') {
      ...
      if ((this.rates?.glucose || 0) > 0) {
        ...
        this._econNegTicks = 0; // 恢复正收益后重置负收益计数
      } else {
        this._evoEconTimer = Math.max(0, (this._evoEconTimer || 0) - 0.5); // 负收益时缓慢倒退
        this._econNegTicks = (this._econNegTicks || 0) + 1;
        // ★ 负收益5秒 → 日志提醒，点破"能量够≠葡萄糖够"
        if (this._econNegTicks === 5) {
          const eRate = this.rates?.energy || 0;
          if (eRate > 0.5) {
            this.log('💡 ATP能量充足但葡萄糖在跌！培养任务需要 🍬葡萄糖 保持正收益 — 试试多造碳源采集器', 'w');
          } else {
            this.log('⚠ 葡萄糖收支为负，培养任务计时停滞 — 多造碳源采集器或减少耗糖建筑', 'w');
          }
        }
        // ★ 负收益15秒 → 引导气泡指向葡萄糖资源卡片
        if (this._econNegTicks === 15) {
          const glucoseCard = document.getElementById('resCard-glucose');
          if (glucoseCard) {
            this._showGuideBubble('🍬 葡萄糖在跌！多造碳源采集器', glucoseCard, { dir: 'right', duration: 6000 });
          }
        }
      }
    }
  ...
  _updateEvoTaskUI() {
    ...
    // 经济任务负收益提示（强化版：直接点破瓶颈资源）
    let stallHint = '';
    if (task.id === 'et1_econ30' && (this.rates?.glucose || 0) <= 0) {
      const gRate = this.rates?.glucose || 0;
      const eRate = this.rates?.energy || 0;
      // ★ 精准诊断：如果能量在涨但葡萄糖在跌，说明玩家可能误判
      const misreadHint = eRate > 0
        ? `<div style="font-size:0.65em;color:#eab308;margin-top:2px">💡 ATP能量充足≠葡萄糖充足！此任务看的是 <b style="color:#22c55e">🍬葡萄糖</b> 速率</div>`
        : '';
      stallHint = `<div style="font-size:0.72em;color:#f97316;margin-top:3px;padding:4px 6px;background:rgba(249,115,22,0.08);border-radius:4px;border-left:2px solid #f97316">⚠ 葡萄糖 <b>${formatRate(gRate)}/s</b> — 需要正收益才能计时<br><span style="font-size:0.9em">→ 多造 <b style="color:#22c55e">碳源采集器</b> 或减少耗糖建筑</span>${misreadHint}</div>`;
    }
    // 停滞免费跳过提示
    if (isFreeSkip) {
  ...
  _showEvoTaskSelection(tasks) {
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      html += `<div class="evo-task-option${met?' met':''}" data-evo-idx="${i}" style="
        ...
        ${(t.id === 'et1_econ30' && !met && (self.rates?.glucose || 0) <= 0) ? `<div style="font-size:0.68em;color:#f97316;margin-top:4px;padding:2px 6px;background:rgba(249,115,22,0.06);border-radius:3px">⚠ 当前葡萄糖 ${formatRate(self.rates?.glucose||0)}/s — 选此任务需先让葡萄糖转正</div>` : ''}
      </div>`;
    }
    html += '</div>';
  ...
  _doPhaseUpgrade() {
    ...
    // ★ v3.1 C2改: P1→P2 和 P2→P3 都将自动管线转手动（避免线突然消失吓到玩家）
    if ((this.phase === 1 || this.phase === 2) && this._computeBelts) {
      ...
      if (converted > 0) {
        if (this.phase === 1) {
          this.log(`🔗 ${converted}条传送带已转为手动模式`, 's');
        } else {
          // ★ P2→P3: 记录哪些管线是从自动转来的（用于后续优化提醒）
          this._p3ConvertedBelts = this._p3ConvertedBelts || new Set();
          for (const belt of belts) {
            if (belt.isManual) continue;
            const key = Math.min(belt.fi, belt.ti) + '-' + Math.max(belt.fi, belt.ti);
            if (this.manualBelts[key]) this._p3ConvertedBelts.add(key);
          }
          this._p3ConvertedBintLastRemind = 0; // 上次提醒时间戳
          this.log(`🔗 ${converted}条自动管线已保留为手动连接 — 新建筑需自己连线！`, 's');
        }
      }
    }
    // 清理旧的遗留管线机制（已废弃，改为全转手动）
    this._p3LegacyBelts = null;
  ...
  animLoop() {
    ...
    const computeBelts = () => {
      ...
      if (beltMode === 'auto' || beltMode === 'hybrid' || beltMode === 'degraded') {
  ...
  updateAlertBar() {
    ...
    // ── 2d. P3旧管线优化提醒（每5分钟提醒一次，直到玩家全部优化） ──
    if (this.phase >= 3 && this._p3ConvertedBelts && this._p3ConvertedBelts.size > 0) {
      // 清理已被玩家删除的旧管线key
      for (const key of [...this._p3ConvertedBelts]) {
        if (!this.manualBelts[key]) this._p3ConvertedBelts.delete(key);
      }
      if (this._p3ConvertedBelts.size > 0) {
        const now = Date.now();
        const lastRemind = this._p3ConvertedBintLastRemind || 0;
        const interval = 300000; // 5分钟
        const firstDelay = 60000; // 进入P3后60秒首次
        const entryTime = this._p3EntryTime || 0;
        if (now - entryTime > firstDelay && now - lastRemind > interval) {
          alerts.push({
            icon: '🔧', severity: 'info',
            text: `${this._p3ConvertedBelts.size}条旧管线可优化 — 管线面板中标"旧"的可重连`
          });
          this._p3ConvertedBintLastRemind = now;
        }
      }
    }
  ...
  save(silent) {
    ...
    try {
      const s = {
        ...
        _p3ConvertedBelts: this._p3ConvertedBelts ? [...this._p3ConvertedBelts] : null,
        _p3ConvertedBintLastRemind: this._p3ConvertedBintLastRemind || 0,
        // v2.0: 教学flag持久化
        ...
        _p3LegacyBelts: null, // v3.1改: 已废弃，旧管线在升级时直接转手动
  ...
  load() {
    try {
      ...
      if (s._p3ConvertedBelts) this._p3ConvertedBelts = new Set(s._p3ConvertedBelts);
      if (s._p3ConvertedBintLastRemind) this._p3ConvertedBintLastRemind = s._p3ConvertedBintLastRemind;
      // v2.0: 恢复教学flag
      ...
      // v3.1改: 旧存档的遗留管线转为手动管线
      if (s._p3LegacyBelts && typeof s._p3LegacyBelts === 'object') {
        for (const [key, legacy] of Object.entries(s._p3LegacyBelts)) {
          if (!this.manualBelts[key] && legacy.fi !== undefined && legacy.ti !== undefined) {
            this.manualBelts[key] = {
              fi: legacy.fi, ti: legacy.ti,
              colors: legacy.colors || [], icons: legacy.icons || [], labels: legacy.labels || []
            };
          }
        }
        this._p3LegacyBelts = null;
      ...
      // v3.1改: P3遗留管线退化已废弃（旧存档在上方已转为手动管线）
  ...
  // ★ v3.1 C2: [已废弃] 遗留管线退化序列 — 现在P2→P3时直接全转手动
  // 保留此函数以兼容可能残留的旧调用引用
  _startLegacyDecay() {
...
// ====================================================================
// ===== 游戏术语 Tooltip 系统 — 已拆分至 tooltip.js =====
// ====================================================================
// const GameTooltip 定义在 tooltip.js 中，通过 <script src="tooltip.js"> 加载
// ===== BOOT =====
