...
| # | 高优问题 | 本文档解决方案 | 章节 |
|---|---------|--------------|------|
| **1** | 能量ATP是唯一瓶颈（只有2种产出建筑） | 新增P3级"生物燃料电池"建筑 + 端口数值配合 | §2, §3 |
...
```
bioFuelCell: { maxIn: 2, maxOut: 2, role: 'bypass' }
```
...
```javascript
// 新增邻接规则
{ self:'bioFuelCell', neighbor:'biofilmReactor',   bonus:0.15, name:'燃料直供', icon:'🧱', stackable:true, maxStack:2, phase:3 },
{ self:'bioFuelCell', neighbor:'proteinFactory',    bonus:0.12, name:'蛋白催化', icon:'🧪', stackable:true, maxStack:1, phase:3 },
{ self:'bioFuelCell', neighbor:'energyStation',     bonus:0.08, name:'能量串联', icon:'⚡', stackable:true, maxStack:1, phase:3 },
{ self:'bioFuelCell', neighbor:'bioFuelCell',        bonus:0.10, name:'燃料矩阵', icon:'🔋', stackable:true, maxStack:2, phase:3 },
// 反向：生物燃料电池作为邻居对其他建筑的增益
{ self:'biofilmReactor', neighbor:'bioFuelCell',    bonus:0.08, name:'能量回流', icon:'🔋', stackable:true, maxStack:1, phase:3 },
{ self:'sporeSower',     neighbor:'bioFuelCell',    bonus:0.10, name:'燃料驱动', icon:'🔋', stackable:true, maxStack:1, phase:3 },
```
...
```javascript
// 生物燃料电池的输入
{ from:'biofilmReactor',   to:'bioFuelCell', res:'biomass',  icon:'🧱', color:'#10b981', label:'生物质' },
{ from:'biomassConverter', to:'bioFuelCell', res:'biomass',  icon:'🧱', color:'#10b981', label:'生物质' },
{ from:'nanoAssembler',    to:'bioFuelCell', res:'biomass',  icon:'🧱', color:'#10b981', label:'生物质' },
{ from:'proteinFactory',   to:'bioFuelCell', res:'protein',  icon:'🧪', color:'#ec4899', label:'蛋白质' },
{ from:'aminoSynth',       to:'bioFuelCell', res:'protein',  icon:'🧪', color:'#ec4899', label:'蛋白质' },

// 生物燃料电池的输出（能量供给所有消耗能量的建筑）
{ from:'bioFuelCell', to:'simpleExtractor',  res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'nitrogenFixer',    res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'proteinFactory',   res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'geneExtractor',    res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'ribosomeCluster',  res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'biofilmReactor',   res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'sporeSower',       res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'biomassConverter', res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'quantumExtractor', res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'qsController',     res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'nanoAssembler',    res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'pheromoneStation',  res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
{ from:'bioFuelCell', to:'resonanceChamber', res:'energy', icon:'⚡', color:'#f59e0b', label:'ATP' },
```

### 2.7 SVG图标

```javascript
bioFuelCell:`<svg viewBox="0 0 64 64" fill="none"><rect x="10" y="12" width="44" height="40" rx="3" fill="#1a1205" stroke="#f59e0b" stroke-width="1.5"/><rect x="18" y="20" width="28" height="16" rx="2" fill="#f59e0b" opacity="0.06" stroke="#f59e0b" stroke-width="0.8"/><path d="M24 24l3 0-1.5 5 3 0-5 9 1.5-5.5-3 0z" fill="#f59e0b" opacity="0.6"/><path d="M36 24l3 0-1.5 5 3 0-5 9 1.5-5.5-3 0z" fill="#f59e0b" opacity="0.4"/><circle cx="22" cy="42" r="3" fill="#10b981" opacity="0.2" stroke="#10b981" stroke-width="0.6"/><text x="22" y="44" text-anchor="middle" fill="#10b981" font-size="4" opacity="0.6">🧱</text><circle cx="42" cy="42" r="3" fill="#ec4899" opacity="0.2" stroke="#ec4899" stroke-width="0.6"/><text x="42" y="44" text-anchor="middle" fill="#ec4899" font-size="4" opacity="0.6">🧪</text><rect x="16" y="38" width="32" height="2" rx="1" fill="#f59e0b" opacity="0.15"/><rect x="16" y="38" width="20" height="2" rx="1" fill="#f59e0b" opacity="0.35"><animate attributeName="width" values="8;32;8" dur="3s" repeatCount="indefinite"/></rect></svg>`,
```
...
改造后（加入1台生物燃料电池）：
```
能量供给：3×ATP合成酶(7.5⚡) + 1×能量缓冲池(1.8⚡) + 1×生物燃料电池(2.0⚡) = 11.3⚡/s
能量消耗：~11.1⚡/s
...
| `bioFuelCell` | 🆕 生物燃料电池 | P3 | 旁路 | **2** | **2** | 能量旁路例外，同能量缓冲池规则 |
| **P4 — 自动化** |
...
- [ ] `game.js` BLDS — 新增 `bioFuelCell` 建筑定义
- [ ] `game.js` — 新增 `PORT_DEFS` 常量表（22项）
- [ ] `game.js` — 新增 `getUsedPorts()` / `hasAvailablePort()` / `getPortConfig()`
- [ ] `game.js` — 新增 `getPortEfficiencyDiscount()` 端口效率折扣函数
- [ ] `game.js` — 新增 `previewAdjacencyBonuses()` 邻接预览函数
- [ ] `game.js` — 新增 `isEffectivelyAdjacent()` 有效邻接判定函数
- [ ] `game.js` SVG — 新增 `bioFuelCell` 的SVG图标
- [ ] `game.js` FLOW_MAP — 新增 `bioFuelCell` 的所有输入/输出流关系
- [ ] `game.js` ADJACENCY_RULES — 新增 `bioFuelCell` 相关6条邻接规则
- [ ] `game.js` MUTATIONS — 新增 `branchPipeline`(稀有) 和 `hubNode`(史诗) 突变
- [ ] `game.js` 转生加成 — 新增 `portEngineering` 无限加成

### 13.2 经济系统改动

- [ ] `game.js` 维护费计算 — 改为葡萄糖为主（§7.1.1）
- [ ] `game.js` 管理开销 — 阈值8→12，幅度6%→4%，上限180%→160%（§7.1.2）
- [ ] `game.js` 资源竞争 — 阈值75%→85%，最低效率50%→60%（§7.1.3）
- [ ] `game.js` 资源竞争 — 新增P3缓冲期180秒渐进阈值
- [ ] `game.js` 维护费计算 — 整合端口效率折扣（§7.1.4）

### 13.3 科技系统改动

- [ ] `game.js` TECHS `rapidMetabolism` — 去掉消耗惩罚，增加邻接条件bonus（§6.1）
- [ ] `game.js` TECHS `adaptiveLogistics` — 增加+1输出端口+传送带容量+30%（§6.1）

### 13.4 解锁节奏改动

- [ ] `game.js` MUT_LAB_CONFIG — `unlockEvoLv: 3 → 4`（§8.3）
- [ ] `game.js` MUT_LAB_CONFIG — 新增 `unlockExtraReq` 条件
- [ ] `game.js` — 新增P3缓冲期逻辑（竞争阈值渐进）
- [ ] `game.js` — P2 hybrid传送带模式（自动+可选手动）（§8.4）
- [ ] `game.js` — 新增分散解锁的事件触发时间点

### 13.5 传送带逻辑改动

- [ ] `game.js` `computeBelts()` — 自动连接增加端口余量检查
- [ ] `game.js` 手动连接 — 增加 `hasAvailablePort` 检查
- [ ] `game.js` 手动断开 — 更新端口状态
- [ ] `game.js` `getBeltMultiplierForBuilding()` — 验证不超过端口上限

### 13.6 渲染层改动

- [ ] `game.js` `renderGrid()` — 端口渲染（半圆/箭头/颜色状态）
- [ ] `game.js` `renderGrid()` — 邻接预览高亮（放置模式）
- [ ] `game.js` `renderGrid()` — 已激活邻接计数标记
- [ ] `game.js` — 移除呼吸光环 `aura`
- [ ] `game.js` — L型色标→顶部细线
- [ ] `game.js` tooltip — 增加端口信息 + 邻接信息 + 端口折扣
- [ ] `index.html` — 端口样式（.port.in / .port.out / .port.linked / .port.idle）
- [ ] `index.html` — 邻接预览样式（.adj-preview-1/2/3）
- [ ] `index.html` — .cell.ports-full 样式

### 13.7 教学与引导

- [ ] `game.js` GUIDE — P2阶段增加端口+邻接教学消息
- [ ] `game.js` GUIDE — P3阶段分阶段教学消息（传送带→竞争→变异）
- [ ] `game.js` — P2升级时的端口教学面板
- [ ] `game.js` — P3升级时的物流时代教学面板
- [ ] `game.js` — P3+3min资源竞争启用教学面板
- [ ] `game.js` — 首次端口满一次性提示
- [ ] `game.js` — 首次邻接放置推荐提示
...
| 生物燃料电池太强导致ATP合成酶过时 | 中 | 产出刻意低于ATP合成酶(2.0 vs 2.5)，且消耗的生物质/蛋白质是P3稀缺资源 |
