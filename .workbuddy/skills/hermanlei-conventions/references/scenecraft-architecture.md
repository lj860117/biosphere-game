...
> **最后更新**：2026-04-10（P4 版本自动检测、scenelightmanager 合并、DetectP4Version/RunP4Direct、login -s stdout+stderr 合并）  
> **源码位置**：`G:\Project_15hb\client\unity\Assets\ArtOnly\SceneCraft\Editor\`  
> **命名空间**：`SceneCraftKit`  
> **文件数量**：29 个 .cs 文件（+2 scenelightmanager，-1 SceneEffect 待删除，-1 multilightchecker 待删除）  
> **总代码量**：约 30,000+ 行
...
└── 独立工具
    ...
    ├── scenelightmanager.cs             场景光照管理窗口框架 + 场景效果切换 Tab
    └── scenelightmanager.checker.cs     灯光检查 Tab（MultiLight 体素可视化）
```
...
> **所有新增工具 / 独立工具的入口统一在 Hub 的 `EnsureHubToolsRegistered()` 中注册卡片。**
> 不要自己注册 `MenuItem("Tools/SceneCraft/xxx")`。

**原因**：`Tools/SceneCraft` 是场景工坊主入口（快捷键 `%#p`），一旦有子菜单就会变成子菜单展开，导致主入口按钮失效。

**规则汇总**：
1. 新工具在 `_hubTools.Add(...)` 中注册卡片（名称 + 描述 + 打开方法）
2. `SceneCraft/` 顶级菜单下的 MenuItem（如场景效果切换、快速切换阶段）**不影响** `Tools/SceneCraft`，可以继续用
3. 外部工具注册需 hermanlei 审核、author 必填、文件放 `External/` 子目录（详见 Hub.cs 底部注释）
4. **绝不在 `Tools/SceneCraft/` 下注册子 MenuItem**
...
**P4 chcp 65001 包装 + 版本自动检测（2026-04-10）：**
- 旧版 p4（< 2020）：`RunP4Command` 走 `cmd /c "chcp 65001 >nul 2>&1 && p4 args..."` 包装（解决 GBK 假阳性，详见 pitfalls P4-2）
- 新版 p4（≥ 2020，如 2023.2）：直接调 p4.exe，不加 chcp（新版 + chcp 不兼容，详见 pitfalls P4-3）
- **DetectP4Version()**：首次调用 RunP4Command 时执行 `p4 -V`（不包 chcp），解析版本年份，设 `_p4NeedsChcp` 标志
- **RunP4Direct()**：`login -s` 等不涉及中文路径 digest 的命令直接调 p4.exe，绕过版本判断（新旧版都安全）
- `P4LoginWithPassword`：直接调 p4.exe（不包 chcp），因为 cmd.exe 中间层可能消费 stdin 导致 p4 login 收不到密码
- 切换 p4 路径时重置 `_p4VersionChecked = false`（3 处路径切换点）
- StandardOutputEncoding / StandardErrorEncoding 设为 UTF-8
- CreateNoWindow 恢复为 true（不再闪黑窗口）
- `login -s` 同时读 stdout + stderr 合并判断（新版 p4 将结果输出到 stderr，详见 pitfalls P4-4）
...
### 4.8 scenelightmanager — 场景光照管理工具

**文件**：`scenelightmanager.cs`（窗口框架 + 场景效果切换 Tab）+ `scenelightmanager.checker.cs`（灯光检查 Tab）

**核心类：** `scenelightmanager : EditorWindow`（partial class，命名空间 SceneCraftKit）

**功能概述**：合并了原 `SceneCraftWindow.SceneEffect.cs`（场景效果切换）和 `multilightchecker.cs`（MultiLight 灯光检查器）为统一窗口。

**菜单入口**（不变）：
```csharp
[MenuItem("SceneCraft/场景效果切换 (打开面板)")]
static void ShowWindow();
[MenuItem("SceneCraft/快速切换阶段 (循环) %#l")]
static void QuickCycleLightingScenario();
```

**Tab 结构**：
- **Tab 0：场景效果切换** — APV Lighting Scenario 切换、TOD 控制、Volume 参数存储/加载（原 SceneEffect 功能）
- **Tab 1：灯光检查** — MultiLight 体素可视化、灯光数量/范围检查

**灯光检查 Tab 核心功能**：

```csharp
void DrawCheckerTab();                    // 灯光检查页签主绘制
void ScanScene();                         // 扫描场景中所有 MultiLight 相关灯光
void OnCheckerSceneGUI(SceneView sv);     // SceneView 叠加绘制（体素框 + 网格线）
void DrawVoxelBoxes(CheckerScanResult r); // 绘制体素框（LOD0 实线、LOD1 虚线）
void DrawLodGrid(CheckerScanResult r);    // 绘制 LOD 覆盖范围网格线
void DrawDashedWireCube(...);             // 虚线体素框绘制
```

**关键数据结构**：
```csharp
class CheckerScanResult {
    Vector3 cameraPos;          // 扫描时的相机位置
    Vector3 lod0GridOrigin;     // LOD0 网格原点
    Vector3 lod1GridOrigin;     // LOD1 网格原点
    List<VoxelInfo> lod0Voxels; // LOD0（小范围灯）体素列表
    List<VoxelInfo> lod1Voxels; // LOD1（大范围灯）体素列表
}

struct VoxelInfo {
    Vector3Int index;    // 体素坐标
    int lightCount;      // 该体素内灯光数量
    bool isOverLimit;    // 是否超出 32 灯上限
}
```

**体素可视化设计**：
- LOD0（小范围灯，Range ≤ 8）：实线体素框，颜色 绿/黄/红
- LOD1（大范围灯，Range > 8）：虚线体素框，颜色 青蓝/橙/品红
- 体素框标签显示 "[小范围] N/32" 或 "[大范围] N/32"
- 正常体素纯线框，问题体素半透明填充
- 体素框跟随 Scene View 相机移动（32m snap 边界自动重扫描）
- 可独立显示/隐藏 LOD0 和 LOD1（`_showLod0` / `_showLod1`）
- LOD 覆盖范围外框 + 相机所在 Y 层的 XZ 切面网格线

**新增功能**：
- Timeline 特效灯光检测（通过 PlayableDirector 绑定识别）
- "打开 Light Explorer" 快捷按钮

**Hub 卡片**：原"场景效果切换"改为"场景光照管理"

**旧文件（待删除）**：`SceneCraftWindow.SceneEffect.cs`、`multilightchecker.cs`（已被合并）

**与 MultiLight 系统的关系**：
- 读取 MultiLight 的体素结构（通过 EcoEngine.Runtime.dll 反射访问 MultiLightSystem）
- 不修改 MultiLight 参数，只做只读检查
- 全局 255 灯、每体素 32 灯、阴影灯 10 盏的限制来自 Shader 硬编码
...
P4SubmitChecker
    ...
    └── 新增字段 → _p4NeedsChcp / _p4VersionChecked（版本自动检测）
...
scenelightmanager（partial: .cs + .checker.cs）
    ├── 反射 → EcoEngine.Runtime.dll   （MultiLightSystem 体素读取）
    ├── 引用 → SceneCraftStyles.cs
    └── 引用 → SceneCraftSettings.cs   （配置持久化）

SceneCraftHub
    └── 引用各工具窗口的 ShowWindow() 方法（含 scenelightmanager）
...
| P4 版本兼容 | DetectP4Version 自动检测 + 分策略执行 | 旧版(< 2020)需 chcp 65001 包装（P4-2），新版(≥ 2020)不兼容 chcp（P4-3），版本检测是唯一同时满足两者的方案 |
| 场景效果+灯光检查 | 合并为 scenelightmanager | 两者都围绕灯光/光照，合成统一窗口减少入口混乱 |
...
| 场景效果切换 / TOD | `scenelightmanager.cs` Tab 0 |
| MultiLight 灯光检查 / 体素可视化 | `scenelightmanager.checker.cs` Tab 1 |
| P4 版本自动检测 | `P4SubmitChecker.cs` DetectP4Version / RunP4Direct |
