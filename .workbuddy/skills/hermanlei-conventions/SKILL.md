...
| 你在做什么 | 必读 | 建议读 | 不需要读 |
|-----------|------|--------|---------|
| **写新 Unity 编辑器工具** | 零（闭环流程）、二（编辑器偏好）、三（接口设计） | 七（C# 编码规范）、八（如果是 SceneCraft） | 五（P4）、渲染知识索引 |
| **重构/改老代码** | 一（重构铁律，🔴 必读！）、七（编码规范） | 零的④增量实施、八（如果是 SceneCraft） | 二（编辑器偏好）、渲染知识索引 |
...
| **做游戏（biosphere-game）** | 一（重构铁律）、七（编码规范） | `references/gamedev-knowledge-index.md`（如果存在） | 二（编辑器偏好）、八（SceneCraft） |
