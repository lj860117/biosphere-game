...
##### 机制二：每月前沿调研（知识扩展）— 自动化任务，每月 1 日 10:00
...
| 方向 | 搜索什么 | 更新什么 |
|------|----------|----------|
| **Unity 编辑器工具** | Unity 博客新公告、编辑器 API 变更、UI Toolkit 进展 | `SKILL.md` 中的工具开发规范 |
| **渲染/Shader** | SIGGRAPH/GDC 演讲、NVIDIA/AMD 新技术、Unity 渲染管线更新 | `references/rendering-knowledge-index.md` |
| **编程范式/工程实践** | 新的架构模式、AI 辅助开发新趋势、代码质量工具 | `SKILL.md` 中的编码规范和闭环流程 |

**流程**：
1. AI 搜索以上三个方向（每个方向 2~3 个关键词搜索）
2. 筛选出**对当前项目有实际价值**的发现（不是所有新东西都值得关注）
3. 输出「本月前沿调研报告」：每个发现标注 ⭐ 值得采纳 / 👀 值得关注 / ℹ️ 了解即可
4. 用户确认后，⭐ 级的写入 Skill 或知识索引

**评估标准**（决定一个新技术/实践是否值得采纳）：
1. **跟我们的场景相关吗？** 不相关 → 跳过（比如纯主机优化对编辑器工具没用）
...
Skill 内的所有内容分为四层，每层的更新频率和重要性不同：

| 层级 | 内容 | 特点 | 更新频率 | 在 SKILL.md 中的位置 |
|------|------|------|----------|---------------------|
| **🔴 铁律层** | 重构铁律、安全重构四件套、备份检查单 | **违反必出事**，绝不妥协 | 极少更新，除非踩了新坑 | 第一章 |
| **🟡 规范层** | 编码规范、命名规范、闭环流程、架构原则 | **遵守能提质**，是好习惯 | 每次发现更好的实践时更新 | 第零章、第七章 |
| **🟢 知识层** | 渲染知识索引、技术选型建议、前沿技术图谱 | **参考能提效**，是工具箱 | 每月前沿调研时更新 | references/ 目录 |
| **⚪ 约定层** | 项目路径、Git 规范、快捷指令 | **遵守能统一**，是约定 | 项目变化时更新 | 第四~六章 |

**分层的好处**：
- AI 知道什么**必须**遵守（🔴 铁律），什么**应该**遵守（🟡 规范），什么**可以**参考（🟢 知识）
- 新内容加入时有明确的归属——不是随便塞在末尾
- 用户想快速了解"哪些是硬规则"时，看 🔴 就够了
