...
5. **记忆演化（旧条目清理）**：逐条检查 pitfalls.md 和 bug-tracker.md 中的条目是否仍然适用
6. **调研索引刷新**：检查 research-index.md 中的条目，超过 180 天的标注是否需要重新调研

### 记忆演化规则（第 5 步详细说明）

> **灵感来源**：A-MEM (NeurIPS 2025) — 新记忆触发旧记忆修正。
> **核心原则**：知识库不是档案馆，过时的信息比没有信息更危险。

**AI 每月巡检时，对 pitfalls.md 和 bug-tracker.md 逐条执行以下判断：**

| 检查项 | 判断依据 | 操作 |
|--------|---------|------|
| Unity/引擎版本升级后 bug 已修复 | 查 Unity Release Notes 或用户确认 | 标注 `[已过期:Unity X.Y 已修复]` |
| 项目技术栈变更导致不再适用 | 如从 EcoEngine 迁到标准 URP | 标注 `[已过期:技术栈变更]` |
| 同一问题有了更好的解决方案 | 新调研发现更优方案 | 在原条目追加"更优方案见 XXX"，原条目保留不删 |
| 条目创建 > 180 天且无人引用 | 检查 context-triggers 命中记录 | 标注 `[待确认:长期未触发]` 提醒用户决定 |
| bug-tracker 中 `[已修复]` 超过 90 天 | 稳定运行无回归 | 移入"历史归档"区段（不删除） |

**标注格式**：
```markdown
### APV-1：EcoEngine 魔改导致 ProbeReferenceVolume 无法直接引用 [🔴] [已过期:Unity 6.5 已修复]
```

**绝对不做的事**：
- ❌ 不删除任何条目（只标注，保留历史价值）
- ❌ 不自动执行过期标注（列出建议 → 用户确认后才标）
- ❌ 不在紧急任务中途执行记忆演化（完成当前任务后再做）

**过期条目的后续处理**：
- 快速索引表增加"状态"列，过期条目标灰
- context-triggers.md 中对应的触发行不删除，但提醒内容追加"（注意：此条目可能已过期）"
- 每季度（每 3 次月度维护）统计过期率，如果 > 30% 说明知识库需要大规模刷新
